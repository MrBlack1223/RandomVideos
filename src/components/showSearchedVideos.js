...
const ShowSearchedVideo = ()=>{
    ...
    useEffect(()=>{
        const feachSearchedVideos = async ()=>{
            const res = await axios.get(`https://random-videos-api.onrender.com/video/search/search?search=${query}`)
            const channelRes = await axios.get(`https://random-videos-api.onrender.com/user/search?search=${query}`)
